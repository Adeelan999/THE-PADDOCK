...
function showPage(name){if(!pageIds.includes(name))name='drivers';const pages=[...document.querySelectorAll('.page')];pages.forEach(p=>p.classList.remove('page-enter'));pages.forEach(p=>p.classList.toggle('active',p.id==='page-'+name));requestAnimationFrame(()=>document.getElementById('page-'+name)?.classList.add('page-enter'));document.querySelectorAll('[data-page]').forEach(b=>b.classList.toggle('active',b.dataset.page===name));if(name==='calendar')renderSchedule();closeMenu();window.scrollTo({top:0,behavior:'smooth'});}
