...
function updateMemberUI(){
  ...
  const signupTop=$('signupTop'),loginTop=$('loginTop'),topMember=$('topMember'),accountStrip=document.querySelector('.account-strip');
  if(!memberStatus)return;
  if(m){
    memberStatus.innerHTML=`เข้าสู่ระบบแล้ว: <b>${esc(m.username)}</b> • คะแนน ${accountScore(m.username)} • สมาชิกตั้งแต่ ${esc(m.createdAt||'วันนี้')}`;
    loginBtn?.classList.add('hidden');signupBtn?.classList.add('hidden');logoutBtn?.classList.remove('hidden');
    signupTop?.classList.add('hidden');loginTop?.classList.add('hidden');
    if(topMember){topMember.textContent=m.username;topMember.title=`บัญชี ${m.username}`;topMember.classList.remove('hidden')}
    accountStrip?.classList.add('logged-in');
  }else{
    memberStatus.textContent='ยังไม่ได้เข้าสู่ระบบ • กรุณาสมัครบัญชีก่อนหรือเข้าสู่ระบบ';
    loginBtn?.classList.remove('hidden');signupBtn?.classList.remove('hidden');logoutBtn?.classList.add('hidden');
    signupTop?.classList.remove('hidden');loginTop?.classList.remove('hidden');
    topMember?.classList.add('hidden');
    if(topMember){topMember.textContent='';topMember.removeAttribute('title')}
    accountStrip?.classList.remove('logged-in');
  }
}
function openAuth(mode){
  if(!authModal)return;
  authModal.classList.add('show');authModal.setAttribute('aria-hidden','false');document.body.style.overflow='hidden';
  authTitle.textContent=mode==='signup'?'SIGN UP':'LOGIN';authKicker.textContent=mode==='signup'?'CREATE ACCOUNT':'ACCOUNT LOGIN';
...
logoutBtn?.addEventListener('click',()=>{clearSession();gameView.innerHTML='';alert('ออกจากระบบแล้ว')});
...
authForm?.addEventListener('submit',async e=>{
  ...
  if(mode==='signup'){
    ...
    closeAuth();alert('สมัครสมาชิกสำเร็จ กรุณาล็อกอินเพื่อเข้าสู่ระบบ');openAuth('login');authUsername.value=u;authPassword.focus();
  }else{
...
function requireMember(){if(!getCurrentUser()){alert('กรุณาสมัครบัญชีและล็อกอินก่อนจึงสามารถเล่นเกมและบันทึกคะแนนได้');$('member')?.scrollIntoView({behavior:'smooth'});openAuth('signup');return false}return true}
let driverRound=0;
...
function startChampionGame(){
  ...
  savePred.addEventListener('click',()=>{const m=getCurrentUser(),all=readJSON(predKey,{});all[m.username]={season:2026,driver:pd,team:pt,createdAt:new Date().toISOString()};localStorage.setItem(predKey,JSON.stringify(all));addScore('prediction',250);updateMemberUI();alert('ล็อกคำทายแล้ว +250 คะแนน')})
}
